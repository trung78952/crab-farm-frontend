  ...
  <b-row>
    ...
    <b-col lg="6">
      <div class="panel mb-0">
        ...
        <div class="panel-body">
          <b-form-row>
            <b-col md="8"><b-form-group label="Topic"><b-form-input
                  v-model="publishForm.topic" /></b-form-group></b-col>
            <b-col md="2"><b-form-group label="QoS"><b-form-input v-model.number="publishForm.qos" type="number" min="0"
                  max="2" /></b-form-group></b-col>
          ...
          <b-form-group label="Payload JSON">
            <b-form-textarea v-model="payloadText" rows="5" max-rows="20" />
          </b-form-group>
...
export default {
  ...
  data() {
    return {
      ...
      clearedAt: null,
      publishForm: {
        topic: "farm/shelf/SHELF_01/motion/cmd",
        qos: 0,
        retain: false,
      },
      payloadText: '{\n  "type": "home"\n}',
  ...
  computed: {
    topicOptions() {
      return [
        ...
        ...this.topics.map((topic) => ({ value: topic, text: topic })),
      ];
    },
    filteredLogs() {
      ...
      return this.logs
        .concat(
          this.$store.state.mqttLogs.filter(
            (log) =>
              !this.clearedAt || new Date(log.created_at) > this.clearedAt,
          ),
        )
        .filter(
          (log) => !this.selectedTopic || log.topic === this.selectedTopic,
        )
        .filter(
          (log) => this.direction === "all" || log.direction === this.direction,
        )
        .filter(
          (log) =>
            !term ||
